...
int HashInsert(int key, HashSlot hashTable[]) {
    ...
    int i = 0;
    while (1) {
        // get newIndex of the slot
        int newIndex = (index_1 + i * index_2) % TABLESIZE;

        // if duplicate key founds
        if (hashTable[newIndex].indicator == USED && hashTable[newIndex].key == key) {
            return -1;
        }
...
int HashDelete(int key, HashSlot hashTable[]) {
    ...
    int index_2 = hash2(key); // get hash2
    while (1) {
        // get newIndex of the slot
        int newIndex = (index_1 + i * index_2) % TABLESIZE;
