...
//            cell_t cell = init_state->cells[pos_from_coords(x, y)];
...
//    }
    ...
    while (!heapq_empty(&pq)) {
        ...
        for (dir = 0; dir < 4; ++dir) {
//            printf("DIR : %d\n", dir);
//            game_print(info, &node_->state);
//            for (size_t y = 0; y < info->size; ++y) {
//                for (size_t x = 0; x < info->size; ++x) {
//                    cell_t cell = node_->state.cells[pos_from_coords(x, y)];
//                    int type = cell_get_type(cell);
//
//                    printf("%d ", type);
//                }
...
//            }
//
//            printf("%d\n", node_->state.num_free);
            if (game_can_move(info, &node_->state, next_color, dir)) {
//                printf("CAN MOVE\n");
//                printf("%d\n", dir);
//                if(dir == 4)
//                    break;
                ...
                //Create child node
                tree_node_t *child = node_create(node_, info, &node_->state);
...
//                Remove node if new position creates a deadend (uncomment code below)
                ...
                game_make_move(info, &child->state, next_color, dir);
