...
if __name__ == '__main__':
    ...
    chapter_bg_color = "white"


    def clearFiles():
        for file in [zip_file_path, output_zip_path, output_doc_path, doc_file_path, "pre_output.docx", "input"]:
    ...
    def changeTitleNames():
        document = Document("pre_output.docx")

        index = 0
        for i in range(len(document.paragraphs)):
            isItHeading = re.match('Heading ([1])', document.paragraphs[i].style.name)
            if isItHeading:
                # print(int(isItHeading.groups()[0]), document.paragraphs[i].text.strip())
                document.paragraphs[i].text = chapter_list[index]
                index += 1
    ...
    def CreateDoc():
        ...
        f.close()
        createFinalDoc("output.docx", save_file_name)
    ...
    def browseFiles():
        ...
        label_file_explorer.configure(text="Document ouvert:" + filename)
        shutil.copy(filename, doc_file_path)

        Button(scrollable_frame, text="Cocher/Décocher", command=selectAll, bg=button_bg_color, fg="white",
               font=('calibre', 10, 'bold'), padx=10, pady=3).pack(pady=20)
        ...
        for i in range(len(order_list) - 1):
            for j in range(0, len(order_list) - i - 1):
                if order_list[j] > order_list[j + 1]:
                    ...
                    chapter_list[j], chapter_list[j + 1] = chapter_list[j + 1], chapter_list[j]
    ...
    def changeChapterNames():
        ...
        try:
            ...
            chapter_list = []
            for chapter_names_var in chapter_names_vars:
                chapter_list.append(chapter_names_var.get())
            ...
            if order_error:
                error = Label(frame_r, text=f"Invalid ordering of the chapters", relief="solid", pady=5,
                              font=("Courier", 10),
                              bg="red")
                error.grid(row=len(chapter_names_vars) + 3, column=1, pady=10)
    ...
    def createChapterNames():
        ...
        for i in range(len(chapter_names)):
            ...
            Label(frame_r, text=f"Chapitre {chapter_names[i][0]}", borderwidth=2, relief="solid", pady=5,
                  font=("Courier", 10),
                  bg=chapter_bg_color).grid(row=i + 3, column=0, pady=10)

            Entry(frame_r, textvariable=chapter_names_vars[i], font=('calibre', 10, 'normal'), width=50).grid(row=i + 3,
                                                                                                              column=1,
                                                                                                              pady=10,
                                                                                                              padx=10)

            Entry(frame_r, textvariable=order_vars[i], font=('calibre', 10, 'normal'), width=5).grid(row=i + 3,
                                                                                                     column=2,
                                                                                                     pady=10)
            chapter_names_vars[i].set(chapter_names[i][1])

        Button(frame_r, text='Appliquer les modifications', command=changeChapterNames, bg=button_bg_color, fg="white",
               font=('calibre', 10, 'bold'), padx=10, pady=3).grid(row=len(chapter_names) + 3, column=0,
                                                                   pady=30)

        # add_new = Button(frame_r, text='Add New Chapter', command=addNewChapter, bg="blue", fg="white",
        #                  font=('calibre', 10, 'bold'), padx=10, pady=3).grid(row=len(chapter_names) + 3, column=1,
        #                                                                      pady=30)
    ...
    def addNewChapter():
        ...
        Label(frame_r, text=f"Nuevo Chapitre", borderwidth=2, relief="solid", pady=5,
              font=("Courier", 10),
              bg=chapter_bg_color).grid(row=position + 1, column=0, pady=10)
    ...
    def showChapters():
        ...
        global main_title_var

        main_title_var = StringVar()

        for widget in frame_r.winfo_children():
            widget.destroy()

        refresh_button = Button(frame_r, text='Afficher', command=showChapters, bg=button_bg_color,
                                fg="white",
        ...
        Entry(frame_r, textvariable=main_title_var, font=('calibre', 10, 'normal'), width=50, bg="light cyan").grid(
            row=1,
            column=1,
            pady=10,
            padx=10)

        Label(frame_r, text="Description", borderwidth=2, relief="solid", pady=5, padx=10,
              font=("Courier", 10),
              bg=chapter_bg_color).grid(row=2, column=0, pady=10)
        description_box = Text(frame_r, height=5, width=52, bg="light cyan")
        description_box.grid(row=2, column=1, columnspan=2)

        createChapterNames()


    def createFinalDoc(content_doc, output_doc):

        # table_of_content_list = []
        # for [index, _] in input_data:
        #     table_of_content_list.append(headers.headers[index])
        #
        # previous = ""
        # current = "A"

        # print("prev content : ", table_of_content_list)
        # for i in range(1, len(table_of_content_list)):
        #     if previous == "":
        #         previous = table_of_content_list[i][0]
        #         table_of_content_list[i][0] = current + table_of_content_list[i][0][1:]
        #         continue
        #     if previous[0] != table_of_content_list[i][0][0]:
        #         current = chr((ord(current) + 1 - 65) % 26 + 65) + current[1:]
        #         previous = table_of_content_list[i][0]
        #         table_of_content_list[i][0] = current + table_of_content_list[i][0][1:]
        #
        #     else:
        ...
        # print("table content : ", table_of_content_list)
        main_title = main_title_var.get()
        ...
        desc_1 = description_box.get(1.0, "end-1c")
        context = {'title': main_title, "table_title": table_title, "desc_1": desc_1,
    ...
    refresh_button = Button(frame_r, text='Afficher', command=showChapters, bg=button_bg_color, fg="white",
                            font=('calibre', 10, 'bold'), padx=10, pady=3).grid(row=0, column=0,
                                                                                pady=30)
    ...
    button_explore = Button(root, text='Inserer le document', command=browseFiles, bg=button_bg_color, fg="white",
                            font=('calibre', 10, 'bold'), padx=10, pady=3)
    ...
    Button(root, text='Générer le document', command=generateXML, bg=button_bg_color, fg="white",
           font=('calibre', 10, 'bold'), padx=10, pady=3).pack(pady=20)
