...
f = open("input_values.txt")
analysing_files = [f.strip() for f in f.readlines() if len(f.strip()) != 0]

dataset = kitti_object('data')  # change the directory
path = "data"
files = [f for f in listdir(path) if
         isfile(join(path, f)) and f.split(".")[-1] == 'bin' and f.split(".")[0] in analysing_files]
