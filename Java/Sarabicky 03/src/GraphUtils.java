...
public class GraphUtils {
    ...
    public static int minDistance(Graph graph, String src, String dest) {
        ...
        Map<String, Integer> min_distances = new HashMap<>();

        for (String node : graph.getAllNodes()) {
            min_distances.put(node, 0);
        }
        ...
        while (!queue.isEmpty()) {
            ...
            ArrayList<String> neighbours = new ArrayList<>(graph.getNodeNeighbors(peek));
            for (int i = 0; i < neighbours.size(); i++) {
                if (!visited.contains(neighbours.get(i))) {

                    if(Objects.equals(neighbours.get(i), dest))
                        return min_distances.get(peek) + 1;
                    min_distances.put(neighbours.get(i), min_distances.get(peek) + 1);
    ...
    public static Set<String> nodesWithinDistance(Graph graph, String src, int distance) {
        ...
        Set<String> nodes_with_distance = new HashSet<>();
        ArrayList<String> visited = new ArrayList<String>();
        Map<String, Integer> min_distances = new HashMap<>();
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < neighbours.size(); i++) {
                if (!visited.contains(neighbours.get(i))) {
                    ...
                    if( min_distances.get(peek) + 1 > distance)
                        return nodes_with_distance;
                    else
                        nodes_with_distance.add(neighbours.get(i));
                }
            }
        }
        return nodes_with_distance;
    ...
    public static boolean isHamiltonianCycle(Graph g, List<String> values) {
        if(g == null || g.getAllNodes().size() < 3 || values == null){
        ...
        for (int i = 1; i < values.size(); i++) {
            if(i != values.size()-1 && visited.contains(values.get(i))){
                ...
                Set<String> neighbours = g.getNodeNeighbors(values.get(i-1));

                if(!neighbours.contains(values.get(i))){
        ...
        if(Objects.equals(values.get(values.size()-1), values.get(0))){
            return true;
        }
