...
{
    int temp_size = list.size_;;
    struct Node * temp = list.head;

    list.size_ = this->size_;
    list.head = this->head;
...
void DoublyLinkedList<ValueType>::Iterator::insertBefore(const ValueType& value)
{
    ...
    if (this->position == nullptr) {
        ...
        this->position->prev = new_node;
        this->position = new_node;
    }else {
        new_node->next = this->position;
        new_node->prev = this->position->prev;
        this->position->prev = new_node;
...
void DoublyLinkedList<ValueType>::Iterator::remove(bool moveToNextAfterward)
{
    ...
    struct Node * temp = this->position;
    
    if(this->position->prev != nullptr)
        this->position->prev->next = this->position->next;

    
    if(this->position->next != nullptr)
