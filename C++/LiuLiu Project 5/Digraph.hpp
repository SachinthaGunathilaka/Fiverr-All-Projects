...
public:
    ...
    // edge in this Digraph.  Only edsges outgoing from the given vertex
...
private:
    ...
    std::map<int, struct DigraphVertex<VertexInfo, EdgeInfo>> digraph;
    ...
    void DFS( const std::map<int, struct DigraphVertex<VertexInfo, EdgeInfo>> & graph, int v, std::map<int, bool> &visited) const
    {
        visited.find(v)->second = true;

    
        for (auto const& edge : digraph.at(v).edges){
            if(!visited.at(edge.toVertex)){
                DFS(graph, edge.toVertex, visited);
            }
        }
    }
};
...
std::vector<std::pair<int, int>> Digraph<VertexInfo, EdgeInfo>::edges() const
{
    ...
    for (auto const& vertex : digraph){
        ...
        for (auto const& edges : digraph.at(vertex.first).edges){
            result.push_back(std::pair<int, int>(edges.fromVertex, edges.toVertex));
        }
    }
...
std::vector<std::pair<int, int>> Digraph<VertexInfo, EdgeInfo>::edges(int vertex) const
{
    ...
    if ( digraph.find(vertex) == digraph.end() ) {
        throw DigraphException("Vertex does not exists");
    }
    std::vector< std::pair<int, int>> result;
    for (auto const& edges : digraph.at(vertex).edges){
        result.push_back(std::pair<int, int>(edges.fromVertex, edges.toVertex));
    }
    return result;
}
...
VertexInfo Digraph<VertexInfo, EdgeInfo>::vertexInfo(int vertex) const
{
    ...
    return digraph.at(vertex).vinfo;
}
...
EdgeInfo Digraph<VertexInfo, EdgeInfo>::edgeInfo(int fromVertex, int toVertex) const
{
    ...
    if ( digraph.find(fromVertex) == digraph.end() || digraph.find(toVertex) == digraph.end()) {
        throw DigraphException("One or more vertices does not exists");
    }

    for (auto const& edge : digraph.at(fromVertex).edges){
...
void Digraph<VertexInfo, EdgeInfo>::addVertex(int vertex, const VertexInfo& vinfo)
{
    ...
    struct DigraphVertex<VertexInfo, EdgeInfo> new_vertex;
    new_vertex.vinfo = vinfo;
    digraph.insert(std::pair<int, struct DigraphVertex<VertexInfo, EdgeInfo>>(vertex, new_vertex));
...
void Digraph<VertexInfo, EdgeInfo>::addEdge(int fromVertex, int toVertex, const EdgeInfo& einfo)
{
    ...
    struct DigraphEdge<EdgeInfo> edge;
    edge.fromVertex = fromVertex;
    edge.toVertex = toVertex;
    edge.einfo = einfo;

    digraph.at(fromVertex).edges.push_back(edge);
...
void Digraph<VertexInfo, EdgeInfo>::removeVertex(int vertex)
{
    ...
    for (auto const& vertices : digraph){

        for (auto const& edge : digraph.at(vertices.first).edges){
...
void Digraph<VertexInfo, EdgeInfo>::removeEdge(int fromVertex, int toVertex)
{
    ...
    if(i == digraph.at(fromVertex).edges.size())
        throw DigraphException("Edge does not exists");
    

    typename std::list<DigraphEdge<EdgeInfo>>::iterator iter;
    iter = digraph.at(i).edges.begin();
    std::advance(iter,i);


    digraph.at(fromVertex).edges.erase(iter);
...
int Digraph<VertexInfo, EdgeInfo>::edgeCount() const noexcept
{
    ...
    for (auto const& vertex : digraph){
        ...
        count += digraph.at(vertex.first).edges.size();
    }
...
int Digraph<VertexInfo, EdgeInfo>::edgeCount(int vertex) const
{
    ...
    return digraph.at(vertex).edges.size();
...
bool Digraph<VertexInfo, EdgeInfo>::isStronglyConnected() const
{
    for (auto const& vertex : digraph){
        ...
        for (auto const& x : digraph)
        {
            visited.insert({x.first, false});
        }
        
        DFS(digraph, vertex.first, visited);
    ...
    std::function<double(const EdgeInfo&)> edgeWeightFunc) const
{
    ...
    std::map<int, double> shortest_distances;
    std::vector<int> unvisited_vertices;
    std::map<int, int> shortest_path;


    for (auto const& vertices : digraph){
        unvisited_vertices.push_back(vertices.first);
        shortest_distances[vertices.first] = INT_MAX;
    
       
    }
    
    shortest_distances[startVertex] = 0;
    shortest_path[startVertex] = startVertex;
    ...
    while(!unvisited_vertices.empty())
    {
        ...
        {
            if(shortest_distances.at(unvisited_vertices[i]) < minimum_value)
            {
                minimum_value = shortest_distances.at(unvisited_vertices[i]);
                minimum_vertex = unvisited_vertices[i];
            }  
        }
    
        for (int i=0;i<unvisited_vertices.size();i++)
        {
            if (unvisited_vertices[i] == minimum_vertex)
            {
                unvisited_vertices.erase(unvisited_vertices.begin()+i);
                break;
            }
        }
        
        for(typename std::list<DigraphEdge<EdgeInfo>>::const_iterator it=digraph.at(minimum_vertex).edges.begin();it!=digraph.at(minimum_vertex).edges.end();it++)
        {
            double distance = shortest_distances[minimum_vertex] + edgeWeightFunc(it->einfo);
            if(distance < shortest_distances[it->toVertex])
            {
                shortest_distances[it->toVertex]= distance;
                shortest_path[it->toVertex] = minimum_vertex;
            }
        }
    }
