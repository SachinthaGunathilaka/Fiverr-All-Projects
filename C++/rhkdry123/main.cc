...
public :
    ...
    Polynomial(std::vector<double> vecCoef, std::vector<int> vecExpo) {
        for (auto element1: vecCoef)
            this->coefVec.push_back(element1);

        for (auto element2: vecExpo)
            this->expoVec.push_back(element2);
    }

    Polynomial operator*(const Polynomial &poly) {
        Polynomial newObject;

        std::vector<double> new_coefVec;
        std::vector<int> new_expoVec;

        for (int i = 0; i < this->expoVec[this->coefVec.size()-1] + poly.expoVec[this->coefVec.size()-1]; ++i) {
            new_expoVec.push_back(i+1);
            new_coefVec.push_back(0);
        }

        for (int i=0; i<this->coefVec.size(); i++)
        {
            for (int j=0; j<poly.coefVec.size(); j++)
                new_coefVec[this->expoVec[i] + poly.expoVec[j] -1] += this->coefVec[i] * poly.coefVec[j];
        }

        for (int i = 0; i < this->expoVec[this->coefVec.size()-1] + poly.expoVec[this->coefVec.size()-1]; ++i) {
            if(new_coefVec[i] != 0){
                newObject.coefVec.push_back(new_coefVec[i]);
                newObject.expoVec.push_back(new_expoVec[i]);
    ...
    void printPolynomial() {
        cout << endl << "Coef Array = [ ";

        for (auto coef: this->coefVec)
            cout << coef << "  ";
        cout << " ]" << endl;

        cout << endl << "Expo Array = [ ";
        for (auto expo: this->expoVec)
            cout << expo << "  ";
        cout << " ]" << endl;
    }
};
...
int main(void) {
    std::vector<int> expo_vect1{2, 4};
    std::vector<double> coef_vect1{5.0, 3.0};

    std::vector<int> expo_vect2{1, 3};
    std::vector<double> coef_vect2{-7, 4};
//
    Polynomial poly1(coef_vect1, expo_vect1);
    Polynomial poly2(coef_vect2, expo_vect2);
