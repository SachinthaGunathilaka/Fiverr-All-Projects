...
typedef struct _Car {
    ...
    _Car(int id, string owner_name, int owner_age, string model, string plate_number) : id(id), ownerName(owner_name),
                                                                                        ownerAge(owner_age),
                                                                                        carModel(model),
...
void readFromFile(Car *cars, int &cur_index, const string &filename) {
    ...
    while (getline(file, line)) {
        ...
        getline(ss, line, ',');
		stringstream ss_temp(line);
		ss_temp >> id;
        getline(ss, ownerName, ',');
        getline(ss, line, ',');
        stringstream ss_temp2(line);
        ss_temp2 >> ownerAge;
        getline(ss, carModel, ',');
        ss >> plateNumber;
...
void searchCar(Car *cars, int cur_index) {
    ...
    for (int i = 0; i < cur_index; i++) {
    	...
    	if(choice == 1){
    		stringstream ss_temp(search_term);
    		ss_temp >> search_term_int;
		}
        if ((choice == 1 && cars[i].id == search_term_int) ||
...
void writeLog(const string &filename, int cur_index){
    ...
    time_t now = time(0);
    char* dt = ctime(&now);
    tm *cur_time = gmtime(&now);
	dt = asctime(cur_time);
	dt[strlen(dt)-1] = '\0';

    ofstream outFile;
    outFile.open(filename.c_str(), ios_base::app);
    outFile << dt << "\t\t" <<  cur_index << " records\t" << endl;
    outFile.close();
